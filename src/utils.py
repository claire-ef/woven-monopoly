...
def get_next_roll(rolls: list[int], current_roll_index: int, is_random: bool = False) -> tuple[int, int]:
    """
    Get the next dice roll. If is_random is True, randomly generate a dice roll. Otherwise load the next dice roll from the preset list.

    Args:
        rolls (list[int]): a list of preset rolls. It is an empty list if is_random is True
        current_roll_index (int): the index of the dice roll
        is_random (bool): whether to generate random dice roll or not. The default is False

    Returns:
        Two integers, the first being the next dice roll, the second being the next dice roll index
    """
    ...
    if current_roll_index >= len(rolls):
        raise IndexError(f'A roll at index {current_roll_index} is requested, but there are only {len(rolls)} rolls given')
